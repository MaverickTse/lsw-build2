...
if [ ! -f bzip2-1.0.6.tar.gz ]; then
    ...
    patch -p1 < bzgrep-debian-1.0.5-6.all.patch
    patch -p1 < bzip2-cygming-1.0.6.src.all.patch
    patch -p1 < bzip2-buildsystem.all.patch
    patch -p1 < bzip2-1.0.6-progress.all.patch
	autoreconf -fi
fi
